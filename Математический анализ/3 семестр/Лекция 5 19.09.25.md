...
**Теорема (эквивалентность пределов по Коши и Гейне):** $\exists\lim_{x \to a} f(x) \Leftrightarrow \forall x^{(k)},x^{(k)} \to a$, $x^{(k)} \ne a \ \exists\lim_{k \to \infty} f(x^{(k)})$, и все такие пределы равны.
